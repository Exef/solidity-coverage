...
class App {
    ...
    try {
      ...
      if (!this.copyNodeModules && this.copyPackages.length) {
        ...
        this.copyPackages.forEach(nodePackage => {
          ...
          shell.cp(
            "-rfL",
            "node_modules/" + nodePackage + "/.",
            this.coverageDir + "/node_modules/" + nodePackage
          );
        });
      }
      ...
      shell.test("-e", `${this.workingDir}/truffle.js`)
        ...
        : (truffleConfig = reqCwd.silent(
            `${this.workingDir}/truffle-config.js`
          ));

      // Coverage network opts specified: use port if declared
      if (
        truffleConfig &&
        truffleConfig.networks &&
        truffleConfig.networks.coverage
      ) {
        ...
        const trufflejs = defaultTruffleConfig(
          this.port,
          gasLimitHex,
          gasPriceHex
        );
   ...
   */
  instrumentTarget() {
    this.skipFiles = this.skipFiles.map(
      contract => `${this.coverageDir}/contracts/${contract}`
    );
    ...
    try {
      shell.ls(`${this.coverageDir}/contracts/**/*.sol`).forEach(file => {
        ...
        if (!this.skipFiles.includes(file) && !this.inSkippedFolder(file)) {
          ...
          const canonicalPath = contractPath
            .split("/coverageEnv")
            .join(working);
          const contract = fs.readFileSync(contractPath).toString();
          const instrumentedContractInfo = getInstrumentedVersion(
            contract,
            canonicalPath
          );
    ...
    shell
      ...
      .forEach(file => {
        ...
        if (
          this.deepSkip &&
          (this.skipFiles.includes(file) || this.inSkippedFolder(file))
        )
          return;
        ...
        if (
          contractProcessed.name &&
          contractProcessed.name === "SyntaxError" &&
          file.slice(-15) !== "SimpleError.sol"
        ) {
          console.log(
            `Warning: The file at ${file} was identified as a Solidity Contract, ` +
  ...
  launchTestrpc() {
    return new Promise((resolve, reject) => {
      if (!this.norpc) {
        const defaultRpcOptions = `--accounts ${this.accounts} --port ${
          this.port
        }`;
        const options =
          (this.testrpcOptions || defaultRpcOptions) +
          ` --gasLimit ${gasLimitHex}`;
        const command = require.resolve(
          "ethereumjs-testrpc-sc-fork/build/cli.node.js"
        );

        // Launch
        const execOpts = { maxBuffer: 1024 * 1024 * 10 };
        this.testrpcProcess = childprocess.exec(
  ...
  modifyArtifacts() {
    shell
      .ls(`${this.coverageDir}${this.buildDirPath}/*.json`)
      .forEach((file, index) => {
        const artifactPath = this.platformNeutralPath(file);
        const artifactRaw = fs.readFileSync(artifactPath);
        const artifact = JSON.parse(artifactRaw);
  ...
  generateReport() {
    ...
    return new Promise((resolve, reject) => {
      ...
      stream.on("error", err =>
        this.cleanUp("Event trace could not be read.\n" + err)
      );
      const reader = readline.createInterface({
        input: stream
      });
      this.events = [];
      reader.on("line", line => this.events.push(line)).on("close", () => {
        // Generate Istanbul report
        try {
          this.coverage.generate(this.events, `${this.workingDir}/contracts`);
          const relativeMapping = this.makeKeysRelative(
